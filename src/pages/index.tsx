...
import { useForm, SubmitHandler, FieldErrors, UseFormRegister, UseFormHandleSubmit } from 'react-hook-form';
...
function EmailPasswordForm({ onSubmit, loading, errors, register }: EmailPasswordFormProps) {
  return (
    <form
      style={{
        display: 'flex',
...
export default function Home() {
  ...
  const [selectedTapeInfo, setSelectedTapeInfo] = useState<TapeInfo | null>(null);
    ...
    if (akord) {
      ...
      items.forEach((item) => {
        if (item.name === 'tapeInfo.json') {
          ...
          tapeInfoPromise = akord.stack.getVersion(tapeInfoId).then(({ data: decryptedTapeInfo }) => {
      ...
      items.forEach((item) => {
        if (item.versions[0].type.startsWith('audio')) {
          ...
          const audioPromise = akord.stack.getVersion(audioId).then(({ data: decryptedAudio }) => {
          ...
          const imagePromise = akord.stack.getVersion(imageId).then(({ data: decryptedImage }) => {
        ...
        >
          ...
          >
            {showVaultIdForm && isAuthenticated ? (
              <VaultSelectionForm tapeInfoOptions={tapeInfoOptions} setSelectedTapeInfo={setSelectedTapeInfo} handleSubmit={handleSubmit} onSubmit={handleVaultSelection} loading={loading} />
            ) : showVaultIdForm ? (
              <EmailPasswordForm onSubmit={handleSubmit(onSubmit)} loading={loading} errors={errors} register={register} />
            ) : (
              <>
                <button
                  style={{
